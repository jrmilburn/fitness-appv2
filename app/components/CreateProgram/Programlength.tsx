...
export default function ProgramLength({ onNext, setProgram, onPrevious }) {
    ...
    return (
        ...
        <div className="w-full max-w-screen-sm mx-auto mt-4 relative flex justify-between sm:block">
                <button
                    onClick={onPrevious}
                    className="sm:absolute sm:left-0 sm:top-0 p-2 text-primary-text hover:text-secondary-text focus:outline-none sm:translate-x-[-100%]"
                >
                    <FiArrowLeft size={24} />
                </button>
        ...
        <form onSubmit={handleSubmit} className="w-full p-4 sm:p-8 bg-background sm:shadow-md rounded-lg mx-auto max-w-screen-sm sm:border-2">
            ...
            <div className="flex flex-col space-y-6 w-full">
                ...
                <input
                    ...
                    className="p-4 border border-border rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none shadow-sm w-[50%] text-primary-text bg-background"
                    value={programName}
                ...
                <button
                    type="submit"
                    className={`p-4 rounded-lg text-primary-text font-semibold ${isFormValid ? 'bg-black hover:bg-gray-900' : 'bg-gray-400 cursor-not-allowed'}`}
